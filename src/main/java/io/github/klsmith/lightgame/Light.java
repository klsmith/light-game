...
public class Light {
    ...
    public synchronized void update() {
        ...
        lightX[0] = game.getPlayer().getX();
        lightY[0] = game.getPlayer().getY();
        for (int i = 0; i < resolution; i++) {
            final double rayDirection = direction - halfSpread + (i * offset);
            final Double2 point = drawMarchingCircles(game.getPlayer().getX(), game.getPlayer().getY(), rayDirection,
                    0);
    ...
    private double shortestDistanceFromPointToWall(double x, double y) {
        ...
        for (Wall wall : game.getLevel().getWalls()) {
            final double wallCenterX = wall.getX() + (wall.getWidth() / 2);
            final double wallCenterY = wall.getY() + (wall.getHeight() / 2);
            final Double2 wallCenter = new Double2(wallCenterX, wallCenterY);
            final double wallSizeX = wall.getWidth() / 2;
            final double wallSizeY = wall.getHeight() / 2;
        ...
        if (null != shape) {
            if (controller.infrared()) {
                ...
                final RadialGradientPaint radialGradient = new RadialGradientPaint(
                        game.getPlayer().getX(), game.getPlayer().getY(), controller.getRange(),
                        new float[] { 0, 1 },
